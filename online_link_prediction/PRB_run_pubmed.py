from baselines.load_data import load_pubmed
...
def construct_adjacency_matrix_from_G(G, num_users, num_class, FULLGRAPH): 
...
def construct_no_edge_graph(G, FULLGRAPH):
    ...
    if FULLGRAPH == False: 
        ...
        ones = np.ones((G_origin.shape[0], 1))  # Create a column of ones
...
if __name__ == '__main__':
    ...
    for i in range(runing_times):  
        ...
        G_origin, G = construct_no_edge_graph(b.edge_index_all, FULLGRAPH)
        if FULLGRAPH == False: 
        ...
        A = construct_adjacency_matrix_from_G(G, num_users, num_class, FULLGRAPH)
        ...
        for t in range(10000): # change to 20000
    ...
    path = os.getcwd()    
    np.save('{}/results/PRB/Non_Full_Graph_Input/pubmed/PRB1_regrets.npy'.format(path), regrets_all)
